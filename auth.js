...
async function registerUser() {
  ...
  const { data, error } = await supabase
    ...
    .insert([{
      ...
      username: username,
      email: email,
      roadbux: 0,   // default for normal users
      role: 'player'
    }]);
...
async function login() {
  ...
  alert("Logged in!");  
  window.location.href = "dashboard.html"; // Redirect to dashboard/home
}
async function registerUser() {
  const username = document.getElementById("username").value;
  const email = document.getElementById("email").value;
  ...
  if (authError) {
    alert(authError.message); // show any signup errors
    return;
  }
  ...
  const { data, error } = await supabase
    ...
    .insert([{
      ...
      username: username,
      email: email,
      roadbux: 0,       // default for new users
      role: 'player'    // default role
    }]);

  if (error) {
    alert(error.message); // show DB insert errors
    return;
  }
